...
# FUNCION PARA ORDENAR 3 NUMEROS PARA TENER EL BEST FIT.
    ...
    def cont_aleatorio(self, cajas_pe):
      ...
      #  print("largo caa" + str(len(cajas_p)))
       ...
       # Guardo los espacios maximales en una variable
    #   aux_esp_max = []
     #  for i in self.EspMax_p:
      #     aux_esp_max.append(i)
        ...
        def complex_fit():
            ...
            def perfect_fit(cajas_p1,esp_max,d):
                response=False
                if d==3:
                    if(cajas_p1.z==esp_max.z2-esp_max.z1) and(cajas_p1.y==esp_max.y2-esp_max.y1) and (cajas_p1.x==esp_max.x2-esp_max.x1):
                        response=True
                elif d==2:
                    if((cajas_p1.z==esp_max.z2-esp_max.z1) and (cajas_p1.y==esp_max.y2-esp_max.y1) and (cajas_p1.x<=esp_max.x2-esp_max.x1)) or ((cajas_p1.z==esp_max.z2-esp_max.z1)and (cajas_p1.x==esp_max.x2-esp_max.x1) and (cajas_p1.y<=esp_max.y2-esp_max.y1)) or ((cajas_p1.y==esp_max.y2-esp_max.y1) and (cajas_p1.x==esp_max.x2-esp_max.x1)and (cajas_p1.z<=esp_max.z2-esp_max.z1)):
                        response=True
                elif d==1:
                    if ((cajas_p1.z == esp_max.z2 - esp_max.z1 and (cajas_p1.y <= esp_max.y2 - esp_max.y1) and (cajas_p1.x <= esp_max.x2 - esp_max.x1)) or ((cajas_p1.y == esp_max.y2 - esp_max.y1) and (cajas_p1.z <= esp_max.z2 - esp_max.z1) and (cajas_p1.x <= esp_max.x2 - esp_max.x1)) or ((cajas_p1.z == esp_max.z2 - esp_max.z1) and (cajas_p1.x <= esp_max.x2 - esp_max.x1) and (cajas_p1.y <= esp_max.y2 - esp_max.y1))):
                        response=True
            ...
            aux_esp_max=[]
            for j in self.EspMax_p:
                aux_esp_max.append(j)    
            
            
            #funcion de empacar
            def empacar(mejor_caja,esp_peq_p):
    ...
    ################################While de perfect fit
            empaque = True 
            
            while len(cajas_p)>0 and empaque == True :
            
                #Recorro buscando un perfect_fit
                dicc={}
                
                for i in cajas_p:
                    for r in i:
                        for j in self.EspMax_p:
                            #voy de atras hacia adelante
                          for d in range (3,0,-1):
                              per_fit=perfect_fit(r,j,d)
                              dicc[r,j,d]=per_fit
                              if per_fit == True:
                                  break
                          if per_fit ==True:
                ...
                #Recorro todas las llaves del dicc
                tres_d=[]
                dos_d=[]
                uno_d=[]
        
                for i in dicc.keys():
                    if dicc[i]==True and i[2]==3:
                        tres_d.append(i)
                    elif dicc[i]==True and i[2]==2:
                        dos_d.append(i)
                    elif dicc[i]==True and i[2]==1:
                        uno_d.append(i)
                        
                #recorro y el primero lo empaco
                mejor_caja=None
                esp_peq=None
                ...
                if len(tres_d)>0:
                  ...
                  #  print(tres_d)
                    esp_peq=tres_d[0][1]
                    mejor_caja=tres_d[0][0]
                elif len(dos_d)>0:
                    esp_peq=dos_d[0][1]
                    mejor_caja=dos_d[0][0]
                elif len(uno_d)>0:
                    esp_peq=uno_d[0][1]
                    mejor_caja=uno_d[0][0]
                
    
                if mejor_caja!=None and esp_peq!=None:
                    empacar(mejor_caja,esp_peq)
                    empaque = True 
    ...
    ##################TERMINO DE PERFECT FIT
    
    ################## INICIO DE ESCOGO EL ESPACIO MAXIMAL EN DONDE MAS CAJAS ME CABEN          
            #si hay cajas a empacar, reviso todas las cajas y todos los espacios maximales
            #escogo la caja y el espacio maximal a empacar en donde el numero de cajas se maximice
            
            
            empaque_2=True
            if len(cajas_p) > 0 and empaque_2==True:
                dicc={}
                for i in cajas_p:
                    for r in i:
                        random_box=r
                        for j in self.EspMax_p:
                        #verifico si la caja cabe en el espacio maximal 
                            if(random_box.z+j.z1 <= j.z2) and (random_box.x+j.x1 <= j.x2) and (random_box.y+j.y1 <= j.y2):    
                                #calculo de cuantas entran en x
                                x_c=math.floor((j.x2-j.x1)/random_box.x)
                                y_c=math.floor((j.y2-j.y1)/random_box.y)
                                z_c=math.floor((j.z2-j.z1)/random_box.z)
                                #calculo de cuantas entran en y
                                #calculo de cuantas entran en z
                                #Cambio el numero de cajas por la utilizacion ampliada
                                #dicc[r,j]=x_c*y_c*z_c  
                                
                                ##Utilizacion ampliada
                                esp_max_vol_p=(j.z2-j.z1)*(j.x2-j.x1)*(j.y2-j.y1)
                                num_cajas=x_c*y_c*z_c
                                vol_caja=random_box.z*random_box.x*random_box.y
                                dicc[r,j]=(vol_caja*num_cajas)/esp_max_vol_p
                            #calculo de cajas por el eje x
                mejor_caja=None
                esp_peq=None
                
                if bool(dicc): 
                    dicc_ord=sorted(dicc.items(), key=lambda x: x[1], reverse=True)
                    if len (dicc_ord)==1:
                        
                        #print(dicc_ord)
                        empaque_items=dicc_ord[0][0]
                       # print("entro aca1111")
                       # print(empaque_items)
                        mejor_caja=empaque_items[0]
                       # print("mejor caja")
                       # print(mejor_caja)
                        esp_peq=empaque_items[1]
                       # print("esp peq")
                       # print(esp_peq)
                       # print(len(self.CajasEmpacadas))
                        empacar(mejor_caja,esp_peq)
                    ...
                    elif dicc_ord[0][1]>dicc_ord[1][1]:
                        #print("entro aca 22")
                        empaque_items=dicc_ord[0][0]
                       # print("entro aca 22")
                       # print(empaque_items)
                        ...
                        #encontrar los elementos maximos iguales 
                        #print("encontrar 2222")
                        
                        cajas_fit=[]
                        esp_fit=[]
                        e_max=dicc_ord[0][1]
                        
                        for w in range(0,len(dicc)):
                            if dicc_ord[w][1]==e_max:
                                cajas_fit.append(dicc_ord[w][0][0])
                                esp_fit.append(dicc_ord[w][0][1])
                        
                        ###BEST FIT ALGORITHM
                        dicc_fit={}
                        for t in range(0,len(cajas_fit)):
                            j=cajas_fit[t]
                            esp_pp=esp_fit[t]
                            resp=self.sort_3(abs(j.x+esp_pp.x1-esp_pp.x2), abs(
                                j.y+esp_pp.y1-esp_pp.y2), abs(j.z+esp_pp.z1-esp_pp.z2))
                            dicc_fit[j,esp_pp] = resp
                            
                        tripletas_ordenadas = sorted(
                            dicc_fit.items(), key=lambda item: item[1])
                        
                        iterador_claves = iter(tripletas_ordenadas)
                        primera_clave = next(iterador_claves)
                      #  valor_primera_clave = dicc[primera_clave]
                      #  print(primera_clave)
                        mejor_caja = primera_clave[0][0]
                      #  print("mejor_caja")
                      #  print(mejor_caja)
                      #  print("llegoao aca")
                        esp_peq = primera_clave[0][1]
                      #  print("llegoao aca2")
                        empacar(mejor_caja,esp_peq)
                        empaque = True 
                        # elimino la caja a guardar
             ...
             #   print("Entro acaaa en donde las cajas son mayores a 0 ")
                ...
                while cajas_a_uti == False:
                    ...
                    while cabe == False:
                        ...
                        #Escogo el espacio maximal con mayor Z1
                       # vol_ini=0
                        esp_peq = None
                        ...
                        for i in aux_esp_max:
                          ...
                          #  print(vol_is)
                            if(max_z1 < i.z1):
                                max_z1 = i.z1
                                esp_peq = i
                            elif (max_z1 == i.z1) and (vol_is <= vol_ini):
                                vol_is=vol_ini
                                esp_peq=i
                                max_z1=i.z1
                        ...
                        if esp_peq != None:
                            ...
                            #    print(tripletas_ordenadas[0])

                                iterador_claves = iter(dicc)
                                primera_clave = next(iterador_claves)
                                # valor_primera_clave = dicc[primera_clave]

                                cajas_fit.append(primera_clave)
        ...
        uti_p = self.utilizacion
        
        if uti_p>0.5:
            if numero_aleatorio <=0.33:
                caja_param_l, esp_max_param_l, esquina_param_l=best_fit_f()
            elif numero_aleatorio >0.33 and numero_aleatorio <=0.67:
                caja_param_l, esp_max_param_l, esquina_param_l=complex_fit()
            else:
                caja_param_l, esp_max_param_l, esquina_param_l=stacking()
        else:
            if numero_aleatorio <=0.16:
                caja_param_l, esp_max_param_l, esquina_param_l=best_fit_f()
            elif numero_aleatorio >0.16 and numero_aleatorio <=0.33:
                caja_param_l, esp_max_param_l, esquina_param_l=complex_fit()
            else:
                caja_param_l, esp_max_param_l, esquina_param_l=stacking()
...
for i in range(0, 2100):
 ...
 #   print(i)
  ...
  #  print(i)
    ...
    cajas_empacadas.to_excel('cut_2_final/buf_'+str(global_buffer)+'_rea_'+str(
...
os.chdir("C:/Users/juan.huertas/OneDrive - LLA/Documentos/Maestria/Tesis/Solution_Resumen/Cut_2_final")



res_data.to_excel('random_algo_uti_max_k_0_0_summary_boxes_buf_'+str(global_buffer) +
